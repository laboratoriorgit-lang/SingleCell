...
![Pre-filter QC violin plots.](assets/qc_prefilter.png){width=30%}
...
![Post-filter QC violin plots.](assets/qc_postfilter.png){width=30%}
...
![UMAP before Harmony correction, colored by sample.](assets/umap_preharmony.png){width=30%}
...
![UMAP after Harmony correction, colored by sample.](assets/umap_postharmony.png){width=30%}
...
\begin{center}
```
![Elbow diagnostic used to inspect candidate cluster numbers.](assets/elbow_plot.png){width=30%} ![Cluster tree across candidate Leiden resolutions.](assets/clustree.png){width=30%}
```{=latex}
\end{center}
```
...
![Final UMAP colored by numbered Seurat clusters.](assets/umap_seuratclusters.png){width=30%}
...
\begin{center}
```
![Marker dotplot after bibliography-based cell-type annotation.](assets/dotplot_annotation_biblio.png){width=30%} ![UMAP colored by bibliography-based cell-type annotation.](assets/umap_annotation_biblio.png){width=30%}
```{=latex}
\end{center}
```
...
![Cluster tree labeled by the dominant bibliography-based annotation in each node.](assets/clustree_annotated.png){width=30%}
...
\begin{center}
```
![Violin plot for the selected gene across all annotated cell types.](assets/vln_gene_AT5G26000.png){width=30%} ![UMAP expression overlay for the selected gene.](assets/feature_gene_AT5G26000.png){width=30%}
```{=latex}
\end{center}
```
...
Cells per cell type (bibliography annotation `celltype_curated`; the three
epidermis hypocotyl subclusters and the still-numeric unannotated clusters
are kept separate at this stage):

| Cell type | Cells |
|---|---:|
| Mesophyll | 1739 |
| Epidermis Cotyledon | 1569 |
| Epidermis Hypocotyl.1 | 1691 |
| Epidermis Hypocotyl.2 | 752 |
| Epidermis Hypocotyl.3 | 498 |
| PHLOEM | 127 |
| ENDO1 | 67 |
| guard cell | 31 |
| 3 | 1586 |
...
Cell types present in only one condition (here cluster `9`, with only 78
cells) are dropped, since a WT vs pifq contrast needs both conditions
represented. Ten cell types are retained.

Cells per pseudo-replicate (first two cell types shown):

| Cell type | Replicate | Cells |
|---|---|---:|
| Mesophyll | pifq_rep1 | 499 |
| Mesophyll | pifq_rep2 | 527 |
| Mesophyll | pifq_rep3 | 488 |
| Mesophyll | WT_rep1 | 72 |
| Mesophyll | WT_rep2 | 76 |
| Mesophyll | WT_rep3 | 77 |
| Epidermis Cotyledon | pifq_rep1 | ... |
| Epidermis Cotyledon | pifq_rep2 | ... |
| ... | ... | ... |
...
AT1G01010    94.04     -1.659           0.294     -5.652   1.6e-08     1.3e-07
AT1G01020   119.88      0.204           0.168      1.212   2.3e-01     3.6e-01
AT1G01030   417.65     -0.605           0.172     -3.521   4.3e-04     1.8e-03
```
...
Cotyledon, Mesophyll, PHLOEM):

```text
# class matrix                          # log2FC matrix
gene_id    EpiCot  Meso  Phlo          gene_id    EpiCot   Meso   Phlo
AT1G01010    -1     -1     0          AT1G01010    -1.90   -1.66    NA
AT1G01060     1      1     0          AT1G01060     1.47    4.55    NA
AT1G01120    -1      1     0          AT1G01120    -1.72    1.22    NA
```
...
All four parts write into the same `results/` folder inside the cloned
repository (mounted as `/workspace/results` in the container), one
subfolder per step. After running the full pipeline once, it looks like
this:
...
```text
results/
|-- 01_qc/               qc_prefilter.pdf, qc_postfilter.pdf,
|                         umap_preharmony.pdf, umap_postharmony.pdf
|-- 02_clustering/        elbow_plot.pdf, clustree2.pdf, umap_seuratclusters.pdf
|-- 03_annotation/        FindAllMarkers.tsv, dotplot_marker_table_annotation_biblio.pdf,
|                         umap_annotation_biblio.pdf, clustree_annotated.pdf
|-- 04_expression/        feature_gene_<gene>.pdf, vln_gene_<gene>.pdf
|-- 05_curation/          umap_grouped.pdf, subcluster_markers_<cluster>.pdf, umap_curated.pdf
|-- objects/              checkpoints: seurat_list_postfilter.rds, ath_sc_preharmony.rds,
|                         ath_sc_postharmony.rds, ath_sc_curated.rds/.h5ad,
|                         pseudobulk_replicates/, ath_sc_pseudotime_final.h5ad
|-- 06_de_results/<tag>/  DESeq2_<cell_type>_<tag>.csv, <cell_type>_<tag>_filtered.csv,
|                         diff_table_<tag>_fc1_padj_005.tsv, log2FC_table_fc1_padj_005.tsv,
|                         heatmap_<tag>.pdf, volcano/DESeq2_<cell_type>_<tag>.png,
|                         volcano/VolcanoPlots_<tag>.pdf
|-- 07_go/<tag>/          GO_<cell_type>_<tag>_BP.tsv, GO_<cell_type>_<tag>_BP_bubble.pdf
|-- 08_networks/          edges_unified.tsv, nodes_unified.tsv, modules_unified.tsv,
|                         hubgenes_unified.tsv, hdwgcna_summary.tsv, unified_TOM.rda,
|                         hdwgcna_unified.rds, network_tf_DE_direction.pdf
`-- 09_pseudotime/        umap_overview.pdf, umap_selection.pdf,
                          trajectory/<run_name>/parameters.tsv,
                          trajectory/<run_name>/<run_name>_{annotation,
                            pseudotime_trajectory,root_cell}.pdf,
                          trajectory/<run_name>/<run_name>_pseudotime_by_cell.tsv,
                          trajectory/<run_name>/<run_name>_trajectory.h5ad,
                          trajectory/<run_name>/gene_plots/<run_name>_gene_plots.pdf,
                          trajectory/<run_name>/gene_trends/step29_gene_trends_
                            {top10,highlight_list}_FINAL_24x18.pdf
```
```{=latex}
\normalsize
```

`01_qc`-`05_curation` and `objects/` come from Part 2 (`step1_singlecell.R`);
`06_de_results`/`07_go` (one subfolder per `comparisons` tag, Section 15) and
`08_networks` from Part 3 (`step2_degs.R`); `09_pseudotime` from Part 4
(`step3_pseudotime.py`), one `trajectory/<run_name>/` per entry in
`TRAJECTORY_RUNS` (Section 26). `<gene>`, `<cluster>`, `<cell_type>`, and
`<tag>` depend on the values used when running each section, not on fixed
names.
